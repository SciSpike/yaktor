...
if [ -n "$(route get $SUBNET | grep 'destination: default')" ]; then
  ...
  mkdir -p /tmp/socket;
  socat -d UNIX-LISTEN:/tmp/socket/${STACK}.sock EXEC:"'docker exec -i ${STACK}_vpn_1 socat STDIO TUN'" & sleep 1
  sudo bin/soctun -t $NET -u /tmp/socket/${STACK}.sock & sleep 1 #-h $VPN_HOST_IP -p $SOCTUN_PORT & sleep 1
